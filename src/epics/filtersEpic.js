...
import { GET_TOPICS_URL, GET_OPP_TYPES_URL, GET_EVENT_TYPES_URL, GET_DISCIPLINES_URL } from '../constants/filters/filtersEndpoints';
import { FETCH_TOPICS, FETCH_OPP_TYPES, FETCH_EVENT_TYPES, FETCH_DISCIPLINES } from '../constants/filters/filtersFetchTypes';
import { SET_TOPICS, SET_OPP_TYPES, SET_EVENT_TYPES, SET_DISCIPLINES } from '../constants/filters/filtersReducerTypes';
...
export const getDisciplineTypesEpic = (action$, store) =>
  action$.ofType(FETCH_DISCIPLINES)
    .switchMap(action =>
      Rx.Observable.ajax({
        url: GET_DISCIPLINES_URL,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + action.token
        }
